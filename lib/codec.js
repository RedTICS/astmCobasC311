...
function decode(data) {
    if (data.startsWith(token.STX)) { // # may be decode message \x02...\x03CS\r\n
        var records = decodeMessage(data);
        return records;
    }
...
function decodeMessage(message) {
    ...
    while (msg.indexOf(token.STX) > -1) {
        ...
        frame = message.slice(0, STXIndex + 1);
        frame = decodeFrame(frame);
        frameMerge.push(frame);

        msg = msg.slice(STXIndex + 1);
        message = message.slice(STXIndex + 1);
    }

    frame = decodeFrame(message); // Last frame(should contains ETX)
    frameMerge.push(frame);

    var records = frameMerge.join("");
...
function decodeFrame(frame) {
    // Decodes ASTM frame 
    frame = frame.slice(1);
    var frame_cs = frame.slice(0, -2);
    frame = frame_cs.slice(0, -2);
    var cs = frame_cs.slice(-2);
    var css = makeChecksum(frame);
    ...
    if (frame.endsWith(token.CR + token.ETX)) {
        frame = frame.slice(0, -2);
    }
    else if (frame.endsWith(token.ETB)) {
        frame = frame.slice(0, -1);
    }
    else {
        throw new Error('Incomplete frame data ' + frame + '. Expected trailing <CR><ETX> or <ETB> chars');
    }
    var seq = frame.slice(0, 1);
    if (!isDigit(seq)) {
        throw new Error('Malformed ASTM frame. Expected leading seq number ' + frame);
    }
...
function encodeRecord(record, encoding) {
    ...
    return fields.join(token.FIELD_SEP); // return FIELD_SEP.join(fields)
}
...
function encodeComponent(component, encoding) {
    ...
    // var regex = new RegExp("\\" + token.REPEAT_SEP + "*$");
    // return items.join(token.COMPONENT_SEP).replace(regex, ""); // TODO Hacer un rstrip COMPONENT_SEP.join(items).rstrip(COMPONENT_SEP) 
    return items.join(token.COMPONENT_SEP); // TODO Hacer un rstrip COMPONENT_SEP.join(items).rstrip(COMPONENT_SEP) 
}
...
function encodeRepeatedComponent(components, encoding) {
    ...
    // var regex = new RegExp("\\" + token.REPEAT_SEP + "*$");
    return items.join(token.REPEAT_SEP); //.replace(regex, "");;
